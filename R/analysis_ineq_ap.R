...
dt <- ap %>% 
  ...
  left_join(nuts3_plot_data,
            by = 'geo')
dt_sf <- sf::st_sf(dt, geometry = dt$geometry)
...
dt <- socioecon_dt_ind %>% 
  ...
  filter(rowSums(is.na(.)) == 0) %>% # TODO fix
  left_join(nuts3_plot_data)
dt_sf <- sf::st_sf(dt, geometry = dt$geometry)

plot_nutsID <- ggplot(data = dt_sf) +
  geom_sf(aes(fill = NUTS_ID)) +
  labs(title = "NUTS3 Regions", fill = "Value") +
  theme_minimal() +
  theme(legend.position = 'none')
ggsave(plot_ap, file = 'figures/plot_nutsID.pdf', height = 10, width = 10, units = 'cm')
